...
public class SimulateWindow extends JDialog implements ActionListener
{
	...
	public void simulateLL(FiniteAutomata fa, Grammar g, List tokenNameList) throws NotLLException
	{	
	...
	public void simulateSLR(FiniteAutomata fa, Grammar g, List tokenNameList)
	{	
		lex.setEnabled(fa != null);
		synt.setEnabled(g != null);
	...
	{	
		...
		try
		{
			...
			while (t != null)
			{
				String name = (String) tokenNameList.get(t.getId() - 2);
	...
	private FiniteAutomata generateTokenListAutomata()
	{
		try
		{
			...
			{
				String token = (String) tokenNameList.get(i);
...
class TokenTableModel extends AbstractTableModel
{
	...
	public Object getValueAt(int rowIndex, int columnIndex)
	{
		switch (columnIndex)
		{
			...
			case 1:
				return ((Token)tokens.get(rowIndex)).getLexeme();
			case 2:
				return new Integer(((Token)tokens.get(rowIndex)).getPosition());
			default:
				return null;
		}
	}
