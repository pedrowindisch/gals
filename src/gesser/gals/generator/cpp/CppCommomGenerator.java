...
public class CppCommomGenerator
{
	...
	public Map generate(FiniteAutomata fa, Grammar g, Options options) throws NotLLException
	{
	...
	private String lexDecls(FiniteAutomata fa, Options options)
	{
		...
		return 
			"const int STATES_COUNT = "+fa.getTransitions().length+";\n"+
	...
	private String syntDecls(Grammar g, Options options)	
	{
		...
		switch (options.parser)
		{
			...
			{		
				...
				{
					int size = g.getProductions().getProd(i).get_rhs().size();
					if (size > maxProd)
						maxProd = size;
				}
	...
	private String lexTables(FiniteAutomata fa, Options options)
	{
		...
		count = fa.getTransitions().length;
