...
public class ConflictSolver extends Box implements LLConflictSolver, LRConflictSolver
{
    ...
    public int resolve(Grammar g, BitSet conflict, int input, int stackTop)
    {        
        String in;
        if (input == 0)
            in = "$";
        ...
        for (int i=conflict.nextSetBit(0); i>=0; i=conflict.nextSetBit(i+1))
        {
            conflictListModel.addElement(new ProductionItem(pl.getProd(i).toString(), i));
        }
